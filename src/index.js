  ...
  createAccessories(platform, receivers) {
    ...
    receivers.forEach(receiver => {
      if (!this.connections[receiver.ip_address]) {
        ...
        this.connections[receiver.ip_address] = eiscp0;
      ...
      new OnkyoAccessory(platform, receiver); // NOSONAR
    });
  }
}
...
class OnkyoAccessory {
  ...
  createRxInput() {
    ...
    for (const exkey in eiscpData) {
      let exKeyClone = exkey.split('').join('');
      let hold = eiscpData[exkey].name.toString();
      if (hold.includes(',')) {
        hold = hold.slice(0, hold.indexOf(','));
      }
      if (exKeyClone.includes('“') || exKeyClone.includes('”')) {
        exKeyClone = exKeyClone.replace(/“/g, '');
        exKeyClone = exKeyClone.replace(/”/g, '');
      }

      if (
        exKeyClone.includes('UP') ||
        exKeyClone.includes('DOWN') ||
        exKeyClone.includes('QSTN')
      ) {
        continue;
      }
      ...
      if (exKeyClone in eiscpData) {
        if ('models' in eiscpData[exKeyClone]) {
          set = eiscpData[exKeyClone].models;
        } else {
          continue;
        }
      } else {
  ...
  eventInput(response) {
    if (response) {
      ...
      const index =
        input !== null ? RxInputs.Inputs.findIndex(i => i.label === input) : -1;
  ...
  remoteKeyPress(button, callback) {
    ...
    if (this.buttons[button]) {
      ...
      this.eiscp.command(this.zone + '.setup=' + press, error => {
        if (error) {
          this.log.error(
  ...
  addSources(service) {
    ...
    return RxInputs.Inputs.map((i, index) => {
      const hapId = index + 1;
